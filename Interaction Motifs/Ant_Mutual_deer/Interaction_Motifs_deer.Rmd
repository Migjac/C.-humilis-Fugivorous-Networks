...
```{r Matrix for LDA}
write.csv(counts, "/Users/apple/desktop/Palmito/Postdoc-Interaction Networks/C.-humilis-Fugivorous-Networks/Interaction Motifs/Ant_Mutual_deer/im_glm_4.csv")
...
# Lower 1−tailed test:
pnorm(z.abs, lower.tail=T) #p-value: 0.8928792
# Upper 1−tailed test:
pnorm(z.abs, lower.tail=F) #p−value: 0.1071208/0.108
#Random pattern
...
# Lower 1−tailed test:
pnorm(z.mu_ant, lower.tail=T) #p-value: 0.02983775/0.53067
# Upper 1−tailed test:
pnorm(z.mu_ant, lower.tail=F) #p−value: 0.9701623
#less than expected

#MUTUALIST
z.mut <- (bs_obs$Mutualist - mean(bs2$Mutualist))/sd(bs2$Mutualist) #-0.1496855
# Lower 1−tailed test:
pnorm(z.mut, lower.tail=T) #p-value: 0.4405064
# Upper 1−tailed test:
pnorm(z.mut, lower.tail=F) #p−value: 0.5594936
#Less than expected

#ANTAGONIST
z.ant <- (bs_obs$Antagonist - mean(bs2$Antagonist))/sd(bs2$Antagonist) #0.741204
# Lower 1−tailed test:
pnorm(z.ant, lower.tail=T) #p-value: 0.9363506
# Upper 1−tailed test:
pnorm(z.ant, lower.tail=F) #p−value: 0.1745464/0.171
#Less than expected

#Antagonist-MW
z.antMW <- (bs_obs$`Antagonist-MW` - mean(bs2$`Antagonist-MW`))/sd(bs2$`Antagonist-MW`) #-2.106286
# Lower 1−tailed test:
pnorm(z.antMW, lower.tail=T) #p-value: 0.01758976/0.007
# Upper 1−tailed test:
pnorm(z.antMW, lower.tail=F) #p−value: 0.9824102/0.993
#Less than expected

#Mutualist-AW
z.mutAW <- (bs_obs$`Mutualist-AW` - mean(bs2$`Mutualist-AW`))/sd(bs2$`Mutualist-AW`) #1.422037
# Lower 1−tailed test:
pnorm(z.mutAW, lower.tail=T) #p-value: 0.9224923/0.709
# Upper 1−tailed test:
pnorm(z.mutAW, lower.tail=F) #p−value: 0.0775077/0.291
#Less than expected
...
four_motifs<-ggplot(total,aes(x=variable,y=value, fill=Data))+
       ...
       geom_errorbar(aes(ymin=value-SD, ymax=value+SD),               #addingg the sd to the bars
                ...
                annotate(geom="text", x=1, y=17, label="p=0.107")+annotate(geom="text", x=2, y=21, label="p=0.441")+annotate(geom="text", x=3, y=28, label="p=0.0298") +annotate(geom="text", x=4, y=21, label="p=0.175")+theme(legend.position = c(0.1,0.9))+theme(legend.title=element_blank())+theme(legend.text=element_text(size=rel(1.1)))+ theme(axis.title.x = element_text(size=15),axis.text.x = element_text(vjust=0.5, size=12))+theme(axis.title.y = element_text(size=15),axis.text.y = element_text(vjust=0.5, size=12))+
...
six_motifs<-ggplot(total,aes(x=variable,y=value, fill=Data))+
       ...
       geom_errorbar(aes(ymin=value-SD, ymax=value+SD),               #addingg the sd to the bars
                ...
                annotate(geom="text", x=1, y=17, label="p=0.108")+annotate(geom="text", x=2, y=21, label="p=0.441")+annotate(geom="text", x=3, y=5, label="p=0.291") +annotate(geom="text", x=4, y=14, label="p=0.531")+annotate(geom="text", x=5, y=14, label="p=0.007")+annotate(geom="text", x=6, y=21, label="p=0.171")+theme(legend.position = c(0.1,0.9))+theme(legend.title=element_blank())+theme(legend.text=element_text(size=rel(1.1)))+ theme(axis.title.x = element_text(size=15),axis.text.x = element_text(vjust=0.2, size=12))+theme(axis.title.y = element_text(size=15),axis.text.y = element_text(vjust=0.5, size=12))+
                theme(axis.text.x = element_text(angle = 0, hjust = 0.5))+scale_fill_manual(values = c("black", "white"))+ 
                theme(panel.background = element_blank(),axis.line = element_line(size = 0.5, linetype = "solid", colour = "black"),panel.border =                                 element_rect(colour = "black", fill=NA, size=0.5))          #formating the backgorund an lines of the graph
