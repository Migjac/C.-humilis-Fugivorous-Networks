...
median1<-0.0618
median_mart<-0.3556214
median_matas<-0.04359917
```
...
#Martinazo
compute_per_category_count <- function(my_matrix, iteration, median_mart, n_row) {
...
# ggplot code
p1 <- ggplot(aes(y = perc, x = localidad), data = cn)
p1 <- p1 + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + geom_jitter(position=position_jitter(width=.2), size=3) + ggtitle("Boxplot con media, 95%CI, valore min. e max.") + xlab("localidad") + ylab("perc")+scale_y_continuous(limits=c(-20, 100))
#-------------------------------------------
...
matas_plot<-ggplot(total,aes(x=variable,y=value, fill=Data))+
       geom_bar(stat="identity",position="dodge", colour="black")+    #position an colors of the bars, (dodge refers to side by side bars)
        xlab("Interaction Motifs")+ylab("Frequency")+ scale_y_continuous(breaks = seq(0, 28,2), limits=c(0, 28))+
       geom_errorbar(aes(ymin=value-SD, ymax=value+SD),               #addingg the sd to the bars
                width=0.2,   position=position_dodge(0.9))+
                scale_x_discrete(limits= c("Mutualist","Mutualist-Antagonist","Antagonist","No_Interaction"))+ 
                annotate(geom="text", x=1, y=24, label="p=0.939")+annotate(geom="text", x=2, y=15, label="p=0.046")+annotate(geom="text", x=3, y=8, label="p=0.915") +annotate(geom="text", x=4, y=17, label="p=0.010")+theme(legend.title=element_blank())+theme(legend.text=element_blank())+ theme(legend.position='none')+theme(axis.title.x = element_text(size=15),axis.text.x = element_text(vjust=0.5, size=12))+theme(axis.title.y = element_text(size=15),axis.text.y = element_text(vjust=0.5, size=12))+
                theme(axis.text.x = element_text(angle = 0, hjust = 0.5))+scale_fill_manual(values = c("black", "white"))+ 
