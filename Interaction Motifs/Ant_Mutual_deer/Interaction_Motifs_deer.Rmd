...
#No interaction
z.abs <- (bs_obs$No_Interaction - mean(bs2$No_Interaction))/sd(bs2$No_Interaction) #0.7411706
# Lower 1−tailed test:
...
# Lower 1−tailed test:
pnorm(z.mu_ant, lower.tail=T) #p-value: 0.05688707
# Upper 1−tailed test:
pnorm(z.mu_ant, lower.tail=F) #p−value: 0.9431129
#Random pattern
...
four_motifs<-ggplot(total,aes(x=variable,y=value, fill=Data))+
       ...
       geom_errorbar(aes(ymin=value-SD, ymax=value+SD),               #addingg the sd to the bars
                ...
                annotate(geom="text", x=1, y=19, label="N.S.")+annotate(geom="text", x=2, y=22, label="N.S.")+annotate(geom="text", x=3, y=24, label="p=0.057") +annotate(geom="text", x=4, y=21, label="p=0.078")+theme(legend.position = c(0.1,0.9))+theme(legend.title=element_blank())+theme(legend.text=element_text(size=rel(1.1)))+ theme(axis.title.x = element_text(size=15),axis.text.x = element_text(vjust=0.5, size=12))+theme(axis.title.y = element_text(size=15),axis.text.y = element_text(vjust=0.5, size=12))+
