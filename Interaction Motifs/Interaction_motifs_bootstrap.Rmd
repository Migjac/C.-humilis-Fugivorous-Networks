...
compute_per_category_count <- function(my_matrix, iteration, median, n_row) {
  counts <- mutate( my_matrix,
    ...
    mutate(.,Motif = if_else( combination == "ant-absent_mut-absent","Absent",
                             ...
                             if_else(combination == "ant-strong_mut-weak", "Mutualism_Antagonism",
                             if_else(combination == "ant-weak_mut-absent", "StrongW_Antagonism", 
                             if_else(combination == "ant-weak_mut-strong", "Mutualism_Antagonism",
...
```{r}
write.csv(counts, "/Users/apple/desktop/Palmito/Postdoc-Interaction Networks/C.-humilis-Fugivorous-Networks/Interaction Motifs/im_glm_colapsed.csv")
```
...
boxplot(boostrap_df$count~boostrap_df$Motif,data=boostrap_df, main="interaction motifs", 
  	xlab="Interaction motifs", ylab="frequency")
```
...
#ABSENT
z.abs <- (bs_obs$Absent - mean(bs2$Absent))/sd(bs2$Absent) #0.7403426
# Lower 1−tailed test:
pnorm(z.abs, lower.tail=T) #p-value: 0.7704539
# Upper 1−tailed test:
pnorm(z.abs, lower.tail=F) #p−value: 0.2295461
#Random pattern

#MUTUALISM_ANTAGONISM
z.mu_ant <- (bs_obs$Mutualism_Antagonism - mean(bs2$Mutualism_Antagonism))/sd(bs2$Mutualism_Antagonism) #-1.580298
# Lower 1−tailed test:
pnorm(z.mu_ant, lower.tail=T) #p-value: 0.05701932
# Upper 1−tailed test:
pnorm(z.mu_ant, lower.tail=F) #p−value: 0.9429807
#Random pattern


#MUTUALISM
z.mut <- (bs_obs$StrongW_Mutualism - mean(bs2$StrongW_Mutualism))/sd(bs2$StrongW_Mutualism) #0.1419059
# Lower 1−tailed test:
pnorm(z.mut, lower.tail=T) #p-value: 0.5564228
# Upper 1−tailed test:
pnorm(z.mut, lower.tail=F) #p−value: 0.4435772
#Random pattern

#ANTAGONISM
z.ant <- (bs_obs$StrongW_Antagonism - mean(bs2$StrongW_Antagonism))/sd(bs2$StrongW_Antagonism) #0.7362032
# Lower 1−tailed test:
pnorm(z.ant, lower.tail=T) #p-value: 0.7691965
# Upper 1−tailed test:
pnorm(z.ant, lower.tail=F) #p−value: 0.2308035
#Random pattern
```
...
im_lda<-subset(im_glm[13:25])        

#Correlation matriz for explanatory variables
cor(im_lda[1:9])

im<-standardize(fruits~inf+size+sincr+fruit_set+exit_pol+del.area+nn+dens,im_lda)
im$scale
...
#Discriminant analysis
im.lda<-lda(sim$motif~ ., data=sim)
im.lda
im_p<-predict(im.lda)
im.lda.p<-predict(sim, newdata=sim[,c(1:10)])$class     #predicted categories
...
im_lda["predict_motif"]<-NA
im_lda$predict_motif<-im.lda1$class

#percent correct for eahc category  
ct <- table(im_lda$Motif, im.lda$class)
