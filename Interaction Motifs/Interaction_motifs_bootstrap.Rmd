...
ggplot(total,aes(x=variable,y=value, fill=Data))+
  ...
  geom_errorbar(aes(ymin=value-SD, ymax=value+SD),
                width=.2,   position=position_dodge(.9))+                  # With of the error bars
                scale_x_discrete(limits= c("Abs", "SM", "WM","SM / WA", "SM / SA", "WM / WA","WM / SA","WA", "SA"))+ 
...
###Creating a new classification for discriminant analysis
```{r}
...
im_lda$Motif<-as.factor(im_lda$Motif)
contrasts(im_lda$Motif)


im.lda<-lda(im_lda$Motif~ ., data=im_lda)
im.lda
im_p<-predict(im.lda)
im.lda.p<-predict(im.lda, newdata=im_lda[,c(1:12)])$class
table(im.lda.p,im_lda[,13])
plot(im.lda)

im.lda1<-lda(im_lda$Motif~ ., data=im_lda,CV=TRUE)
table(im.lda1$class,im_lda[,13])
