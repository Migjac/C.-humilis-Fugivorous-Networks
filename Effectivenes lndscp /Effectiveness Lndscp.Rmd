...
mart_effect<-effectiveness_plot(q1=mart_elong$Effectiveness,q2=mart_elong$recruit, pts.color=mart_elong$i_group,label=mart_elong$animal, nlines=6,lines.breaks = vect1,
                       myxlab= "Handled fruits/Visit",
                       myylab="Plant recruitment")
vect1<-seq(0, 0.012,0.0019 ) 

mart_effect<-effectiveness_plot(q1=mart_elong$Effectiveness,q2=mart_elong$recruit, pts.color=mart_elong$i_group,label=mart_elong$animal, nlines=6,
                       myxlab= "Handled fruits/Visit",
                       myylab="Plant recruitment")
vect1<-seq(0, 0.012,0.0019 ) 
...
#mean and sd for each column ("2" indicates that the selected function will be applied to each column ("1" to rows))
mm_e_mean<-apply(mm_e,2,mean)
mm_e_sd<-apply(mm_e,2,sd)

#Creating long shape with the means of each motif
mm_e_long<-melt(mm_e_mean,id.vars="mean")
colnames(mm_e_long) <- "Effectiveness"
mm_e_long["SE_f"]<-NA  #adding a new column for standard deviation
mm_e_long$SE_f<-mm_e_sd/sqrt(length(mm_e_mean)) #adding the values of standard deviation

#naming the first column
mm_e_long <- add_rownames(mm_e_long, "animal")
